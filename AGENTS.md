...
- Public payment details belong only in `SUPPORT_PAYMENT` in `content/site.ts`. Matthew explicitly authorized the current personal Venmo handle and Zelle-enrolled phone number for public display; do not infer permission to publish or replace any other personal identifier. Preserve the personal-profile purchase guidance for Venmo and recipient-verification warning for Zelle. Payments are optional support for services, not charitable donations.
